...
def train(hyp, opt, device, tb_writer=None):
    ...
    for k, v in model.named_parameters():
        if v.requires_grad:
            if '.bias' in k:
                pg2.append(v)  # biases 卷积bias
            elif '.weight' in k and '.bn' not in k:
                pg1.append(v)  # apply weight decay 卷积权重
            else:
                pg0.append(v)  # all else BN参数
    ...
    if rank in [-1, 0]:
        ...
        plot_labels(labels, save_dir=log_dir)  # 可视化标签
        if tb_writer:
        ...
        if not opt.noautoanchor:
            check_anchors(dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz)  # 自动anchor重计算
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # Loss, 核心操作
                loss, loss_items = compute_loss(pred, targets.to(device), model, imgs)  # scaled by batch_size
                if rank != -1:
...
if __name__ == '__main__':
    ...
    # Evolve hyperparameters (optional)
        ...
        # 对超参进行搜索，采用遗传算法进行搜索
        # 先定义一组默认参数，对网络进行小数据训练，得到指标，然后保存成txt
        # 在第k次迭代时候，对前topk组超参进行变异，然后重新训练，不断进化
        # 对小数据可行，大数据需要比较多的时间，不一定可取
